...
const PromocionesContent = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [promosRes, catRes, lentesRes, accesoriosRes] = await Promise.all([
        ...
        axios.get(withBase(API_CONFIG.ENDPOINTS.ACCESORIOS)),
      ]);
      
      setPromociones(Array.isArray(promosRes.data) ? promosRes.data : []);
      setCategorias(Array.isArray(catRes.data) ? catRes.data : []);
      
      // CAMBIO: Combinar lentes y accesorios en un solo array de productos
      const lentesData = Array.isArray(lentesRes.data) ? lentesRes.data : [];
      const accesoriosData = Array.isArray(accesoriosRes.data) ? accesoriosRes.data : [];
      
      // Agregar tipo para distinguir entre lentes y accesorios
